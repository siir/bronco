...
stop_services() {
  ...
  pkill -f "tsx.*mcp-servers/" 2>/dev/null || true
...
if ! ping -c 1 -W 2 "$HUGO_TS_IP" &>/dev/null; then
  err "Cannot reach Hugo at $HUGO_TS_IP. Is Tailscale up?"
  exit 1
fi
...
if [[ "$NO_RESTORE" == false ]]; then
  ...
  docker compose -f docker-compose.dev.yml exec -T postgres pg_restore -U bronco -d bronco --no-owner --no-acl < "$SNAPSHOT_PATH" >/dev/null 2>&1 || true
...
# Quoted because PROJECT_ROOT may contain spaces (e.g. "Source Code/").
