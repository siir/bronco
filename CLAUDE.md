...
- **Analysis strategies (v1 vs v2)**: Four runners live in parallel files under `services/ticket-analyzer/src/analysis/`: `flat-v1.ts`, `flat-v2.ts`, `orchestrated-v1.ts`, `orchestrated-v2.ts`. `shared.ts` holds version-agnostic primitives only; `v2-knowledge-doc.ts`, `v2-prompts.ts`, and `v2-subtask-tools.ts` hold v2-only helpers (`kd_*` compose, fallback-fill, snapshot, prompt snippets; sub-task tool definitions `finalize_subtask`, `dispatch_subtasks`, `complete_analysis`). v2 is default; v1 is opt-in via AppSetting `analysis-strategy-version`. v1 is historical fidelity (pre-truncation, pre-`kd_*`, pre-sub-task-summary split) and must not be modified to add v2 features. v1 never supported orchestrated re-analysis — the dispatcher redirects re-analysis on v1 orchestrated to v1 flat.
- **Orchestrated-v2 sub-task architecture**: `orchestrated-v2.ts` runs a strategist that dispatches parallel sub-tasks via `dispatch_subtasks`. Each sub-task has its own agentic tool loop and calls `finalize_subtask` to return structured results; the strategist calls `complete_analysis` to conclude. The strategist retains access to `kd_read_section` / `kd_read_toc` for synthesis across iterations. Sub-task tool budget: `SUB_TASK_ITERATION_CAP` iterations per sub-task; `dispatch_subtasks` accepts max 5 sub-tasks per batch. A stall guard detects repeated identical strategist hashes or zero-dispatch turns and writes a `⚠️ Orchestrator stalled` marker to the knowledge doc's root-cause section.
- **GitHub integration** (`IntegrationType.GITHUB`): `ClientIntegration` rows can be client-scoped (`clientId` set) or platform-scoped (`clientId IS NULL`, `label: 'default'`). `mcp-repo` prefers a client-scoped GITHUB integration for HTTPS clone URL injection, then falls back to the platform-default, then SSH. `tool-request-github.ts` reads the platform-default GITHUB integration first, then falls back to the legacy `system-config-github` AppSetting, so deploys not yet migrated keep working. PATs are embedded in clone URLs only in memory; `.git/config` is scrubbed to plain URL after every fetch. Client-scoped and platform-scoped integrations are managed via `POST /api/integrations`; per-client PAT integrations can also be set via the Integrations tab in the client detail view.
...
| `services/ticket-analyzer/src/analysis/v2-subtask-tools.ts` | v2 orchestrated-only tool definitions: `FINALIZE_SUBTASK_TOOL`, `DISPATCH_SUBTASKS_TOOL`, `COMPLETE_ANALYSIS_TOOL` — not exposed to flat-v2, v1 runners, or MCP servers. Also exports `SubTaskStopReason` and `SubTaskRunResult`. |
