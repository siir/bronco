    ...
    <div class="tab-section">
      ...
      <app-data-table
        ...
        <app-data-column key="access" header="Access" [sortable]="false" width="130px">
          <ng-template #cell let-p>
            <span class="chip" [class.chip-admin]="accessLabel(p) === 'Portal Admin'" [class.chip-user]="accessLabel(p) === 'Portal User' || accessLabel(p) === 'Contact'">
              {{ accessLabel(p) }}
            </span>
          </ng-template>
        </app-data-column>
...
export class ClientPeopleTabComponent implements OnInit {
  ...
  accessLabel(p: Person): 'Portal Admin' | 'Portal User' | 'Contact' {
    if (p.userType === 'ADMIN') return 'Portal Admin';
    if (p.userType === 'USER') return 'Portal User';
    return 'Contact';
  }
  ...
  deletePerson(person: Person): void {
    const message = `Delete ${person.name}? This cannot be undone. If they are a follower on tickets, deletion will fail.`;
