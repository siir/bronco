...
export class UserService {
  ...
  createUser(data: { email: string; password: string; name: string; role?: string }): Observable<ControlPanelUser> {
    return this.api.post<ControlPanelUser>('/users', data);
  }

  updateUser(id: string, data: Partial<{ name: string; email: string; role: string; isActive: boolean }>): Observable<ControlPanelUser> {
