...
import type { AgenticToolContext } from './flat-v1.js';

const logger = createLogger('ticket-analyzer');

async function executeOrchestratedSubTaskV1(
  deps: AnalysisDeps,
  ticketId: string,
  clientId: string,
  category: string,
 ...
 */
export async function runOrchestratedV1(
  deps: AnalysisDeps,
  ...
  for (let i = 0; i < orchMaxIterations; i++) {
    ...
    for (const batch of taskBatches) {
      const results = await Promise.allSettled(
        batch.map(task => executeOrchestratedSubTaskV1(deps, ticketId, clientId, category, clientContext, environmentContext, task, agenticTools, mcpIntegrations, repoIdByPrefix, { id: orchestrationId, iteration: i + 1, parentLogId: strategistLogId }, orchModelMap)),
      );
      ...
      for (let j = 0; j < results.length; j++) {
        ...
        if (result.status === 'fulfilled') {
          ...
          try {
            const retryResult = await executeOrchestratedSubTaskV1(deps, ticketId, clientId, category, clientContext, environmentContext, task, agenticTools, mcpIntegrations, repoIdByPrefix, { id: orchestrationId, iteration: i + 1, parentLogId: strategistLogId }, orchModelMap);
            knowledgeDoc += `\n\n#### ${task.prompt} (retry)\n${retryResult.content}`;
